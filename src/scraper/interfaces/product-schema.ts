...
export const ScrapedProductItemSchema = z.object({
    ...
    productUrl: z.string().describe('Direct link or relative link to the product page'),
    imageUrl: z.string().describe('URL to the product image'),
    inStock: z.boolean().default(true).describe('Whether the product is currently in stock'),
});

export const ScrapedProductListSchema = z.object({
    products: z.array(ScrapedProductItemSchema).describe('List of matched products found on the page'),
});
